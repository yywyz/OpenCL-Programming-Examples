...
int main(void)
{
	...
	fp = fopen("pooling.cl", "r");
	kernel_code_size = fread(kernel_src_str, 1, MAX_SOURCE_SIZE, fp);
	fclose(fp);
	program = clCreateProgramWithSource(context, 1, (const char **)&kernel_src_str,
		(const size_t *)&kernel_code_size, &ret);
	ret = clBuildProgram(program, 1, &device_id, NULL, NULL, NULL);
	kernel = clCreateKernel(program, "pooling", &ret);
	...
	data_out = clCreateBuffer(context, CL_MEM_READ_WRITE,
		point_num/4 * sizeof(int), NULL, &ret);
	...
	ret = clSetKernelArg(kernel, 4, sizeof(int), (void *)&point_width);
	//ret = clSetKernelArg(kernel, 5, sizeof(int), (void *)&point_height);
